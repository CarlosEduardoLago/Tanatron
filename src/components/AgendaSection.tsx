...
export function AgendaSection() {
  const hasRealDates = AGENDA.some((e) => e.link || (e.date !== "Em breve" && e.location !== "Novas datas em breve"));
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {!hasRealDates || AGENDA.length === 0 ? (
          ...
          >
            ...
            <p className="mt-2 text-sm text-zinc-500">Novas datas de shows em breve. Acompanhe nossas redes.</p>
          </motion.div>
        ) : (
              ...
              >
                {event.link ? (
                  ...
                  >
                    ...
                    <span className="mt-1 block text-sm text-zinc-400">{event.location}</span>
                  </a>
                ) : (
                  <div className="block rounded-xl border border-zinc-700/80 bg-zinc-900/30 px-5 py-4 sm:px-6 sm:py-5">
                    <span className="block font-medium text-white">{event.date}</span>
                    <span className="mt-1 block text-sm text-zinc-400">{event.location}</span>
                  </div>
                )}
